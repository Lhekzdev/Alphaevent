...
export default function CheckoutPage() {
    ...
    const quantity = isMixed
        ? tickets.reduce((sum, t) => sum + t.quantity, 0)
        : ticket?.quantity || 1;

    const ticketType = isMixed
        ? "Mixed Admission"
        : ticket?.ticketType;

    const subtotal = isMixed
        ? tickets.reduce(
            (sum, t) =>
                sum + Number(t.ticketPrice) * Number(t.quantity),
            0
        )
        : Number(ticket?.ticketPrice || 0) * quantity;

    const serviceFee = subtotal * 0.05;

    const total = subtotal + serviceFee;

    const ticketPrice = isMixed
        ? subtotal
    ...
    console.log("Previous Route:", state?.previousRoute);
    return (
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <p className="text-xs text-gray-500 leading-5">
                                    ...
                                    <Link
                                        to="/TermsSer"
                                        className="font-semibold text-[#123499] hover:underline"
                                    >
                                        Terms of Service
                                    </Link>{" "}
                                    and{" "}
                            ...
                            <button
                                ...
                                className={`w-full py-4 rounded-lg flex items-center justify-center gap-2 font-medium transition ${agreed
                                    ? "bg-[#123499] hover:bg-[#1836a5] text-white"
